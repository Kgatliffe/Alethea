...
createmap<-data.frame(Left, Right,LeftCode, RightCode, LeftReward, RightReward, State, NextState)
View(createmap)
...
detain = 3
move = 1
statemap<-data.frame("State"=paste0(time,".",createmap$State[1]), "Action"="Left", "Reward"=createmap$LeftReward[1], "NextState"=paste0(time-detain,".",createmap$NextState[1]))
nextrow<-data.frame("State"=paste0(time,".",createmap$State[1]), "Action"="Right", "Reward"=createmap$RightReward[1], "NextState"=paste0(time-detain,".",createmap$NextState[1]))
statemap<-rbind(statemap,nextrow)
nextrow<-data.frame("State"=paste0(time,".",createmap$NextState[1]), "Action"="None", "Reward"=0, "NextState"=paste0(time-move,".",createmap$NextState[1]))
statemap<-rbind(statemap,nextrow)
```
...
if(openstate[1]>=detain)
{
        ...
        nextrow2<-data.frame("State"=statedummy, "Action"="Right", "Reward"=createmap$RightReward[k], 
 "NextState"=paste0(timedet,".",createmap$State[k+1]))
} 
        else if(openstate[1]<detain)
{
                nextrow<-data.frame("State"=statedummy, "Action"="Left", "Reward"=createmap$LeftReward[k], "NextState"="End")
        nextrow2<-data.frame("State"=statedummy, "Action"="Right", "Reward"=createmap$RightReward[k], 
 "NextState"="End")
} 
        if(openstate[1]>=move)
